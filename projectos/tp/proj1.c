...
/* Trabalho de IAED, construção de um sistema de logistica */
...
encomenda e_ord[Max_Encomendas];
...
void N(encomenda enco, encomenda g_enco[]){
    ...
    g_enco[ide] = enco;
    g_enco[ide].ide = ide;
    scanf(" %[^:\n]",g_enco[ide].nome_cliente);
    
    printf("Nova encomenda %d %s.\n",ide,g_enco[ide].nome_cliente);
    ide += 1;
}

void V(encomenda g_enco[]){
    int ide2;
    scanf("%d",&ide2);
    if (ide2 >= ide){
...
int Compara_Preco_Enc(encomenda anterior, encomenda posterior){ 
    if (anterior.custo_total < posterior.custo_total){
        return TRUE; 
    }
    else if(anterior.custo_total == posterior.custo_total){ /*Garante que o com menor idp fica em primeiro caso os precos sejam iguais*/
        return anterior.custo_total < posterior.custo_total;
    }
    return FALSE;
}
/* Funcao que compara os precos de dois produtos*/
...
void merge(int b,encomenda aux[],produto auxiliar[], int esquerda, int m, int direita){
    int i, j, k;
    if (b == Preco){
        ...
        for (i = m+1; i > esquerda; i--)
            any3[i-1] = aux[i-1];
        for (j = m; j < direita; j++)
            any3[direita+m-j] = aux[j+1];
        for (k = esquerda; k <= direita; k++)
            if (Compara_Preco_Enc(any3[j], any3[i]) == TRUE)
                aux[k] = any3[j--];
            else
                aux[k] = any3[i++];
    }   
...
void mergesort(int b,encomenda aux[],produto auxiliar[], int esquerda, int direita) {
    int m = (direita+esquerda)/2;
    if (direita <= esquerda)
        return;
    mergesort(b,aux,auxiliar, esquerda, m);
    mergesort(b,aux,auxiliar,m+1, direita);
    merge(b,aux,auxiliar, esquerda, m, direita);
}
...
void l(produto sistema[]){
    ...
    if( idp == Valor_inicial){
        ...
        mergesort(Preco,e_ord,s_ord,Valor_inicial, (idp - 1)); /*Realiza o mergesort com a condicao b == Preco */
        printf("Produtos\n");
...
void L(encomenda g_enco[]){
    ...
    if (ide2 >= ide){   /*Caso nao exista devolve o erro */
        ...
        mergesort(Letra,e_ord,g_enco[ide2].lista,Valor_inicial,(g_enco[ide2].num_produtos- 1)); /*Realiza o mergesort com b == Letra */
        printf("Encomenda %d\n",ide2);
        for (i = 0; i < g_enco[ide2].num_produtos; i++){ /*Percorre a encomenda */ 
...
void Y(){
    int i;int j;
    if( ide == Valor_inicial){
        printf("Encomendas\n");
    } 
    else{
        for(i = 0;i < ide;i++){
            for(j = 0; j < g_enco[i].num_produtos;j++){
                g_enco[i].custo_total += g_enco[i].lista[j].preco * g_enco[i].lista[j].qtd;
            }
        }
        mergesort(-5,g_enco,s_ord,Valor_inicial, (ide - 1)); /*Realiza o mergesort com a condicao b == Preco */
        printf("Encomendas\n");
        for ( i = 0; i < ide; i++){ /* Percorre o sistema */
            printf("* %d %d\n",g_enco[i].ide,g_enco[i].custo_total);
        }
        for(i = 0;i < ide;i++){
            g_enco[i].custo_total = 0;
        }
    }
}
...
int main(){
    ...
    while (Maximo_produtos > Valor_inicial){ /*Ciclo infinito */
        ...
        if (letra_inicial == 'a')
            ...
            L(g_enco);
        else if(letra_inicial == 'V'){
            V(g_enco);
        }    
        else if(letra_inicial == 'Y'){
            Y();
        }
    }
    return 0;
