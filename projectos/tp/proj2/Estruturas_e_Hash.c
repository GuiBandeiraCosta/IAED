...
void EquipaApagaHash(p_equipa equipa,int *max_equipa,int *n_equipas,p_equipa **p_tab_e){
    int i;int key; p_equipa v;
    key = hash(equipa->nome,*max_equipa);
    while((*p_tab_e)[key] != NULL){ /* Procura onde esta o jogo na hashtable */
        if((*p_tab_e)[key] == equipa) break;
        else key = (key + 1) % (*max_equipa);
    }
    (*n_equipas)--;free((*p_tab_e)[key]->nome);free((*p_tab_e)[key]);(*p_tab_e)[key] = NULL; /* Nao da free aqui porque vai dar no apaga lista */
    for(i = (key + 1)% (*max_equipa);(*p_tab_e)[i] != NULL; i = (i + 1) %(*max_equipa),(*n_equipas)--){ 
        /* Re insere todos os elementos a seguir ao elemento retirado */
        v = (*p_tab_e)[i];
        (*p_tab_e)[i] = NULL;
        EquipaInsere(max_equipa,n_equipas,v,p_tab_e);
    }
}
