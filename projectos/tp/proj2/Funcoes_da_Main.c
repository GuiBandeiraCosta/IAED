...
/*Guilherme Costa Alameda ist195586 */
...
void Z(int contador,int *max_equipas,int *n_equipas,p_equipa **p_tab_e,link head){
    char nome[String_Max];p_equipa equipa;link t;int encontrei = 0;
    scanf(" %[^:\n]",nome);
    ...
    if(equipa == NULL){
        ...
        for(t = head; t != NULL; t = t->next){
            if(t->jogo->equipa1 == equipa || t->jogo->equipa2 == equipa){
                encontrei +=1;
                break; /*QUando encontro paro de percorrer */
            }
        }
        if(encontrei > 0){ 
            printf("%d Equipa com jogos.\n",contador);
        }
        else{
            EquipaApagaHash(equipa,max_equipas,n_equipas,p_tab_e);
        }
    }    
}
...
void a(int contador,int *max_jogos,int *max_equipas ,int *n_jogos, p_jogo **p_tab_j,p_equipa **p_tab_e,link *head,link *tail){
    char temp[String_Max];char equipa1[String_Max];char equipa2[String_Max]; char info_s[String_Max];
    int score1;int score2;
    char *nome;char *info;
    scanf(" %[^:\n]:%[^:\n]:%[^:\n]:%d:%d:%[^:\n]",temp,equipa1,equipa2,&score1,&score2,info_s);
    if(JogoProcura(max_jogos,temp,p_tab_j) != NULL){
        ...
        if(pont_e1 == NULL || pont_e2 == NULL){
            ...
            info = __strdup(info_s);jogo->info = info;
...
void p(int contador,int *max_jogos,p_jogo **p_tab_j){
    ...
    if (jogo == NULL){
       printf("%d Jogo inexistente.\n",contador);
        ...
        printf("%d %s %s %s %d %d %s\n",contador,jogo->nome_j,nome_e1,nome_e2,jogo->score1,jogo->score2,jogo->info);
    }
}
...
void l(int contador,link head){
        link t;
        for(t = head; t != NULL; t = t->next){
            printf("%d %s %s %s %d %d %s\n",contador,t->jogo->nome_j,t->jogo->equipa1->nome,t->jogo->equipa2->nome,t->jogo->score1,t->jogo->score2,t->jogo->info);
        }
